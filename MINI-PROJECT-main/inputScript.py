...
def prefix_suffix(url):
    try:
        result = extract(url)
        if result.subdomain.count('-'):
            return 1
        else:
            return -1
    except Exception as e:
        print("Error extracting subdomain and suffix:", e)
        return 0
...
def sub_domain(url):
    try:
        result = extract(url)
        if result.subdomain.count('.') == 0:
            return -1
        elif result.subdomain.count('.') == 1:
            return 0
        else:
            return 1
    except Exception as e:
...
#getting the certificate issuer to later compare with trusted issuer 
        #getting host name
        subDomain, domain, suffix = extract(url).subdomain,extract(url).domain,extract(url).suffix
